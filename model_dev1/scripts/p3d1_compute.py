...
df = pd.read_csv('../data/processed/cosmetic1000.csv')
len(df)
...
X = df.drop('ChemicalCount', axis=1)  
y = df['ChemicalCount']               

# Initialize the StandardScaler
scaler = StandardScaler()
scaler.fit(X) # Fit the scaler to the features
pickle.dump(scaler, open('../models/scaler_cosmetic1000.sav', 'wb')) # Save the scaler for later use
...
pickle.dump(X_train, open('../models/X_train_cosmetic1000.sav', 'wb'))
# Pkle X.columns for later use in explanation
pickle.dump(X.columns, open('../models/X_columns_cosmetic1000.sav', 'wb'))